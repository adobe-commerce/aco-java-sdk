...
public interface Client {
     ...
     * creating categories: - Each category requires a unique `slug` and `source`. - To create
     * parent-child relationships, create the `slug` field in a hierarchical format, for example
     ...
     * the [Create category operation](#operation/createCategories) * **State Recovery**: Categories
     * are restored to their exact state from the time of deletion, including all metadata, family
    ...
    /**
     * Create or replace product layers Create product layers to customize and override base product
     * data for specific contexts, locales, or business requirements. Product layers enable you to:
    ...
    /**
     * Delete product layers Remove specific product layers by SKU and source identifiers. This
     * operation permanently deletes the layer data while preserving the base product. **Use
     * Cases:** - Remove expired seasonal or promotional layers - Clean up test layers after A/B
     * testing completion - Delete locale-specific layers when discontinuing market support - Remove
     * outdated customizations **Important Notes:** - Only the specified layer is deleted; base
     * product and other layers remain intact - Both `sku` and `source` (locale + layer) must match
     * exactly for successful deletion
     *
     ...
     * variants for configurable products in the same request. - Use the `routes` field to set
     * category paths. The `path` value must match an existing category slug, for example
     * `men/clothing`. - Create a route for each category path. For example to include a product in
     * each of the following categories `men`, `men/clothing`, and `men/clothing/pants`, specify
     * three `path` values, one for each category.
     *
