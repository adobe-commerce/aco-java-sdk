...
public class Model400ProcessFeedResponse {
  ...
  * @throws IOException if the JSON Element is invalid with respect to Model400ProcessFeedResponse
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Model400ProcessFeedResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in Model400ProcessFeedResponse is not found in the empty JSON string", Model400ProcessFeedResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Model400ProcessFeedResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Model400ProcessFeedResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!Model400ProcessFeedResponse.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'Model400ProcessFeedResponse' and its subtypes
       }
       ...
       return (TypeAdapter<T>) new TypeAdapter<Model400ProcessFeedResponse>() {
           @Override
           public void write(JsonWriter out, Model400ProcessFeedResponse value) throws IOException {
